...
import static org.eclipse.jetty.util.TypeUtil.isTrue;
...
//    }
            ...
            throws Exception
    {
        ...
        X509CertificateHolder rootCert = new X509CertificateHolder(rootCertBin);
        X509CertificateHolder interCert = new X509CertificateHolder(interCertBin);
        X509CertificateHolder finalCert = new X509CertificateHolder(finalCertBin);



        X509CRLHolder rootCrl = new X509CRLHolder(rootCrlBin);
        X509CRLHolder interCrl =  new X509CRLHolder(interCrlBin);

        CertPath path = new CertPath(new X509CertificateHolder[] { finalCert, interCert });

        JcaX509CertificateConverter jcaX509CertificateConverter = new JcaX509CertificateConverter();

        for (X509CertificateHolder x509CertificateHolder : path.getCertificates()) {
            System.out.println(jcaX509CertificateConverter.getCertificate(x509CertificateHolder).toString());
        }
        ...
        // result = path.validate(new CertPathValidation[]{new ParentCertIssuedValidation(verifier), new BasicConstraintsValidation(), new KeyUsageValidation()});
        CertPathValidationResult result = path.validate(new CertPathValidation[]{new BasicConstraintsValidation(true)});
        ...
        if (!result.isValid())
        {
            ...
            System.out.println(result.getFailingCertIndex());
            System.out.println(result.getFailingRuleIndex());
